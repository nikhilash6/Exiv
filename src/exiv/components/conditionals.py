...
class CONDRegular:
    # standard conditioning, shapes must exactly match
    ...
    def can_concat(self, other):
        ...
        if self.cond.device != other.cond.device:
            app_logger.warning("WARNING: conds not on same device, skipping concat.")
            return False
        return True
...
class CONDNoiseShape(CONDRegular):
    # shapes/area conditioning, inpainting masks, gligen boxes
...
class CONDCrossAttn(CONDRegular):
    # for cross attn layers, prompt embeds
    def can_concat(self, other):
        s1 = self.cond.shape
        s2 = other.cond.shape
        if s1 != s2:
